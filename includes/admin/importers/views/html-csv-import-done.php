...
<div class="wc-progress-form-content woocommerce-importer">
	<section class="woocommerce-importer-done">
		...
		if ( 0 < $imported ) {
			$results[] = sprintf(
				...
				_n( '%s imported', '%s imported', $imported, 'woocommerce' ),
		...
		if ( 0 < $updated ) {
			$results[] = sprintf(
				/* translators: %d: products count */
				_n( '%s updated', '%s updated', $updated, 'woocommerce' ),
		...
		if ( 0 < $skipped ) {
			$results[] = sprintf(
				/* translators: %d: products count */
				_n( '%s was skipped', '%s were skipped', $skipped, 'woocommerce' ),
		...
		if ( 0 < $failed ) {
			$results [] = sprintf(
				/* translators: %d: products count */
				_n( 'Failed to import %s product', 'Failed to import %s ', $failed, 'woocommerce' ),
	...
	<section class="wc-importer-error-log" style="display:none">
		<table class="widefat wc-importer-error-log-table">
			<thead>
				<tr>
					<th><?php esc_html_e( ucfirst($this->import_type), 'woocommerce' ); ?></th>
	...
	<div class="wc-actions">
		<a class="button button-primary" href="<?php echo esc_url( admin_url( $this->import_page ) ); ?>"><?php esc_html_e( 'View result', 'woocommerce' ); ?></a>
	</div>
</div>
