...
abstract class WC_Product_Importer implements WC_Importer_Interface {
	...
	protected function process_item( $data ) {
		try {
			...
			if ( 'variation' === $object->get_type() ) {
				...
				/* KOSCH HERE (MULTI ATTRIBUTES) */
				/* $this->set_product_data( $object, $data ); */
					$is_multi=false;
					if (isset($data['raw_attributes'])){
						foreach ( $data['raw_attributes'] as $attribute ) {
							if(is_countable($attribute)
								&& strpos($attribute['name'], ',') !== false
								&& is_countable(explode(",",strtolower($attribute['name'])))
								&& count(explode(",",strtolower($attribute['name'])))>0
							){
								$is_multi=true;
							}
						}
					...
					if ($is_multi){
						$this->set_product_data_multi( $object, $data );
					}else{
						$this->set_product_data( $object, $data );
					}
			}

			$this->set_image_data_external( $object, $data );
	...
	protected function set_image_data_external( &$product, $data ) {
		// Image URLs need converting to IDs before inserting.
		
		//if ( isset( $data['sku'] ) ) {
			//$url="/img/products/".$data['sku'].".png";
		//}
		...
		/* KOSCH HERE (THUMBNAILS) */
		$product->update_meta_data( '_knawatfibu_url', $url );
		//update_post_meta(  $product->get_id(), '_knawatfibu_url', $url );
		//add_metadata( 'post',  $product->get_id(), '_knawatfibu_url',$url, True );
	}
	...
	/* KOSCH HERE (MULTI ATTRIBUTES) */
		protected function set_product_data_multi( &$product, $data ) {
			if ( isset($data['raw_attributes'])) {
				$attributes          = array();
				...
				foreach ( $data['raw_attributes'] as $attribute ) {
					...
					if(array_key_exists('name', $attribute)){
						$attr_names=explode(",",strtolower($attribute['name']));
					}
					if(array_key_exists('value', $attribute)){
						$attr_values=$attribute['value'];
					}
					if(array_key_exists('visible', $attribute)){
						$attr_visible=$attribute['visible'];
					}
					if(array_key_exists('taxonomy', $attribute)){
						$attr_taxonomy=$attribute['taxonomy'];
					}
					if(array_key_exists('default', $attribute)){
						$attr_defaults=$attribute['default'];
					}

					$attr_count=count($attr_names);

					for ($i = 0; $i<$attr_count; $i++) {		
						if(isset($attr_names)){
							if($i>count($attr_names)){
								$attribute['name']=null;
							}else{
								$attribute['name']=ucwords($attr_names[$i]);
							}
						}
						if(isset($attr_values)){
							if($i>count($attr_values)){
								$attribute['value']=null;
							}else{
								$attribute['value']=$attr_values[$i];
							}
						}
						if(isset($attr_visible)){
							if($i<=count($attr_visible)){
								$attribute['visible']=null;
							}else{
								$attribute['visible']=$attr_visible[$i];
							}
						}
						if(isset($attr_taxonomy)){
							if($i>count($attr_taxonomy)){
								$attr_taxonomy[$i]=null;
							}else{
								$attribute['taxonomy']=$attr_taxonomy[$i];
							}
						}
						if(isset($attr_defaults)){
							if($i>count($attr_defaults)){
								$attribute['default']=null;
							}else{
								$attribute['default']=$attr_defaults[$i];
							}
						}
