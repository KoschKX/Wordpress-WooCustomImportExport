...
if ( ! class_exists( 'WC_Product_Importer', false ) ) {
	include_once WOOIMPORTEXPORT_DIR . '/includes//import/abstract/abstract-wc-product-importer.php';
}
	...
	protected function expand_data( $data ) {
		...
		foreach ( $data as $key => $value ) {
			if ( $this->starts_with( $key, 'attributes:name' ) ) {
				...
				$values=explode(",",strtolower($value));
				for ($i = 0; $i<count($values); $i++) {
					$attributes[$i]['taxonomy'] = wc_string_to_bool( $values[$i] );
				}
				unset( $data[ $key ] );
			} elseif ( $this->starts_with( $key, 'attributes:visible' ) ) {
				$values=explode(",",strtolower($value));
				for ($i = 0; $i<count($values); $i++) {
					$attributes[$i]['visible'] = wc_string_to_bool( $values[$i] );
				}
				unset( $data[ $key ] );
			} elseif ( $this->starts_with( $key, 'attributes:default' ) ) {
				$values=explode(",",strtolower($value));
				for ($i = 0; $i<count($values); $i++) {
					$attributes[$i]['default'] = wc_string_to_bool( $values[$i] );
				}
				unset( $data[ $key ] );
