...
<form class="wc-progress-form-content woocommerce-importer" enctype="multipart/form-data" method="post">
	<header>
		<h2><?php esc_html_e( ucfirst($this->import_type).' Import (CSV)', 'woocommerce' ); ?></h2>
		<p><?php esc_html_e( 'This tool allows you to import (or merge) '.$this->import_type.' data to your store from a CSV or TXT file.', 'woocommerce' ); ?></p>
	</header>
	<section>
		<table class="form-table woocommerce-importer-options">
			<tbody>
				...
				<tr>
					<th><label for="woocommerce-importer-update-existing"><?php esc_html_e( 'Update existing '.$this->import_type, 'woocommerce' ); ?></label><br/></th>
					<td>
						<input type="hidden" name="update_existing" value="0" />
						<input type="checkbox" id="woocommerce-importer-update-existing" name="update_existing" value="1" />
						<label for="woocommerce-importer-update-existing"><?php esc_html_e( 'Existing '.$this->import_type.' that match by ID will be updated. '.ucfirst($this->import_type).' that do not exist will be skipped.', 'woocommerce' ); ?></label>
					</td>
				</tr>
